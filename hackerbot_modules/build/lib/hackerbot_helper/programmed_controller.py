...
class ProgrammedController(MainController):
    ...
    def move_head(self, angle):
        try:
            ...
            super().send_raw_command(f"MOVEHEAD,{angle}")
            # Not fetching json response since machine mode not implemented
            return True
        except Exception as e:
    ...
    def move_head_pitch(self, angle):
        try:
            self.check_head_control()
            super().send_raw_command(f"TILTHEAD,{angle}")
            # Not fetching json response since machine mode not implemented
            return True
        except Exception as e:
            self.log_error(f"Error in tilt_head: {e}")
            return False
        
    def set_idle_mode(self):
        try:
            self.check_head_control()
            super().send_raw_command("IDLE")
            # Not fetching json response since machine mode not implemented
            return True
        except Exception as e:
    ...
    def set_gaze(self, x, y):
        try:
            self.check_system()
            super().send_raw_command(f"GAZE,{x},{y}")
            # Not fetching json response since machine mode not implemented
            return True
        except Exception as e:
